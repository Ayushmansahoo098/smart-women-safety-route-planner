...
// Loads Leaflet from CDN once, then creates the map + draws the route.
// ══════════════════════════════════════════════════════════════════════════
// Route styles: index 0 = green (safest), 1 = orange (moderate), 2 = red (risky)
...
function LeafletMap({ routes, activeRoute, originCoords, destinationCoords, fromName, toName }) {
    ...
    const routeLayersRef = useRef([]);   // Array of L.GeoJSON layers, one per route
    const markersRef = useRef([]);       // origin + destination markers
    ...
    useEffect(() => {
        ...
        // Fix broken default icon URLs in Vite environment
    ...
    // ── Step 3: Draw ALL 3 routes whenever routes array changes ────────
    useEffect(() => {
        ...
        // Draw routes in reverse order: red first (bottom), then orange, then green on top
        ...
        drawOrder.forEach((idx) => {
            ...
            // Shadow/glow layer drawn first (wider, more transparent)
            ...
            routeLayersRef.current.push(glowLayer, routeLayer);

            if (!allBounds) allBounds = routeLayer.getBounds();
            else allBounds.extend(routeLayer.getBounds());
        });
        ...
        const mOrigin = L.marker(originCoords, { icon: originIcon })
            .addTo(map)
            .bindPopup(`<b>Start:</b> ${fromName}`, { className: "sr-popup" });
        const mDest = L.marker(destinationCoords, { icon: destIcon })
            .addTo(map)
            .bindPopup(`<b>Destination:</b> ${toName}`, { className: "sr-popup" });
        markersRef.current = [mOrigin, mDest];

        // Fit map to show all routes
        if (allBounds) map.fitBounds(allBounds, { padding: [50, 50] });

    }, [routes, activeRoute, originCoords, destinationCoords, fromName, toName, leafletReady]); // eslint-disable-line react-hooks/exhaustive-deps

    // ── Zoom buttons ───────────────────────────────────────────────────
    const zoomIn  = () => mapRef.current?.zoomIn();
    const zoomOut = () => mapRef.current?.zoomOut();

    // Primary route info for the summary card (safest = index 0)
...
/**
 * Props:
 *  activeRoute  — index 0/1/2 (for decorative mode)
 *  routeResult  — null | { geojson, originCoords, destinationCoords,
 *                           fromName, toName,
